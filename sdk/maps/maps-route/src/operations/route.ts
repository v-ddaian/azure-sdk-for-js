...
import {
  ...
  ResponseFormat,
  RoutePostRouteMatrixOptionalParams,
  RoutePostRouteMatrixResponse,
  RoutePostRouteMatrixSyncOptionalParams,
  RoutePostRouteMatrixSyncResponse,
  TextFormat,
  ...
  BatchRequestBody,
  RoutePostRouteDirectionsBatchOptionalParams,
  RoutePostRouteDirectionsBatchResponse,
  RoutePostRouteDirectionsBatchSyncOptionalParams,
...
export class RouteImpl implements Route {
   ...
   * @param postRouteMatrixRequestBody The matrix of origin and destination coordinates to compute the
   *                                   route distance, travel time and other summary for each cell of the matrix based on the input
   *                                   parameters. The minimum and the maximum cell count supported are 1 and **700** for async and **100**
   *                                   for sync respectively. For example, it can be 35 origins and 20 destinations or 25 origins and 25
   *                                   destinations for async API.
   * @param options The options parameters.
   */
  async beginPostRouteMatrix(
    format: ResponseFormat,
    postRouteMatrixRequestBody: PostRouteMatrixRequestBody,
    options?: RoutePostRouteMatrixOptionalParams
  ): Promise<
    PollerLike<
      PollOperationState<RoutePostRouteMatrixResponse>,
      RoutePostRouteMatrixResponse
    >
  > {
    ...
    ): Promise<RoutePostRouteMatrixResponse> => {
   ...
   * @param options The options parameters.
   */
  async beginPostRouteMatrixAndWait(
    format: ResponseFormat,
    postRouteMatrixRequestBody: PostRouteMatrixRequestBody,
    options?: RoutePostRouteMatrixOptionalParams
  ): Promise<RoutePostRouteMatrixResponse> {
    const poller = await this.beginPostRouteMatrix(
      format,
      postRouteMatrixRequestBody,
      options
    );
  ...
  postRouteMatrixSync(
    format: ResponseFormat,
    postRouteMatrixRequestBody: PostRouteMatrixRequestBody,
    options?: RoutePostRouteMatrixSyncOptionalParams
  ): Promise<RoutePostRouteMatrixSyncResponse> {
    return this.client.sendOperationRequest(
      { format, postRouteMatrixRequestBody, options },
      postRouteMatrixSyncOperationSpec
    );
  }
   ...
   * @param query The Coordinates through which the route is calculated, delimited by a colon.  A minimum
   *              of two coordinates is required.  The first one is the origin and the last is the destination of the
   *              route. Optional coordinates in-between act as WayPoints in the route.  You can pass up to 150
   *              WayPoints.
   * @param options The options parameters.
   */
  getRouteDirections(
   ...
   * the concepts and parameters involved.
   * @param format Desired format of the response. Value can be either _json_ or _xml_.
   * @param query The Coordinates through which the route is calculated. Needs two coordinates at least.
   *              Delimited by colon. First one is the origin and the last is the destination of the route. The
   *              coordinates are in a  lat,long format. Optional coordinates in between act as WayPoints in the
   *              route.
   * @param postRouteDirectionsRequestBody Used for reconstructing a route and for calculating zero or
   *                                       more alternative routes to this reference route.  The provided sequence of coordinates is used as
   *                                       input for route reconstruction. The alternative routes  are calculated between the origin and
   *                                       destination points specified in the base path parameter locations.  If both minDeviationDistance and
  ...
  postRouteDirections(
    ...
    options?: RoutePostRouteDirectionsOptionalParams
  ): Promise<RoutePostRouteDirectionsResponse> {
    return this.client.sendOperationRequest(
      { format, query, postRouteDirectionsRequestBody, options },
   ...
   * API](https://docs.microsoft.com/en-us/rest/api/maps/route/getroutedirections) using just a single
   * API call. You can call Route Directions Batch API to run either asynchronously (async) or
   * synchronously (sync). The async API allows caller to batch up to **700** queries and sync API up to
   * **100** queries.
   ...
   * The string values in the _route directions_ query must be properly escaped (e.g. " character should
   ...
   * ```
   * @param format Desired format of the response. Only `json` format is supported.
   * @param postRouteDirectionsBatchRequestBody The list of route directions queries/requests to process.
   *                                            The list can contain  a max of 700 queries for async and 100 queries for sync version and must
   *                                            contain at least 1 query.
   * @param options The options parameters.
   */
  async beginPostRouteDirectionsBatch(
    format: ResponseFormat,
    postRouteDirectionsBatchRequestBody: BatchRequestBody,
    options?: RoutePostRouteDirectionsBatchOptionalParams
  ): Promise<
    PollerLike<
      PollOperationState<RoutePostRouteDirectionsBatchResponse>,
      RoutePostRouteDirectionsBatchResponse
    >
  > {
    const directSendOperation = async (
      args: coreClient.OperationArguments,
      spec: coreClient.OperationSpec
    ): Promise<RoutePostRouteDirectionsBatchResponse> => {
      return this.client.sendOperationRequest(args, spec);
    };
    const sendOperation = async (
      args: coreClient.OperationArguments,
    ...
    return new LROPoller(
      { intervalInMs: options?.updateIntervalInMs },
      { format, postRouteDirectionsBatchRequestBody, options },
      postRouteDirectionsBatchOperationSpec,
      sendOperation,
      "location"
    );
  }
   ...
   * @param options The options parameters.
   */
  async beginPostRouteDirectionsBatchAndWait(
    format: ResponseFormat,
    postRouteDirectionsBatchRequestBody: BatchRequestBody,
    options?: RoutePostRouteDirectionsBatchOptionalParams
  ): Promise<RoutePostRouteDirectionsBatchResponse> {
    const poller = await this.beginPostRouteDirectionsBatch(
      format,
  ...
  postRouteDirectionsBatchSync(
    format: ResponseFormat,
    postRouteDirectionsBatchRequestBody: BatchRequestBody,
    options?: RoutePostRouteDirectionsBatchSyncOptionalParams
  ): Promise<RoutePostRouteDirectionsBatchSyncResponse> {
    return this.client.sendOperationRequest(
      { format, postRouteDirectionsBatchRequestBody, options },
      postRouteDirectionsBatchSyncOperationSpec
    );
  }
}
...
const postRouteMatrixOperationSpec: coreClient.OperationSpec = {
  path: "/route/matrix/{format}",
  httpMethod: "POST",
  responses: {
    200: {
      bodyMapper: Mappers.RouteMatrixResponse
    },
    201: {
      bodyMapper: Mappers.RouteMatrixResponse
    },
  ...
  requestBody: Parameters.postRouteMatrixRequestBody,
  queryParameters: [
    Parameters.apiVersion,
    Parameters.waitForResults,
    Parameters.computeTravelTimeFor,
...
const postRouteMatrixSyncOperationSpec: coreClient.OperationSpec = {
...
const getRouteDirectionsOperationSpec: coreClient.OperationSpec = {
  ...
  queryParameters: [
    ...
    Parameters.language,
    Parameters.computeBestOrder,
    Parameters.routeRepresentation,
    Parameters.vehicleHeading,
    Parameters.report,
    Parameters.vehicleCommercial,
    ...
    Parameters.fuelEnergyDensityInMJoulesPerLiter,
...
const postRouteDirectionsOperationSpec: coreClient.OperationSpec = {
  path: "/route/directions/{format}",
  httpMethod: "POST",
  responses: {
    200: {
  ...
  requestBody: Parameters.postRouteDirectionsRequestBody,
  queryParameters: [
    Parameters.apiVersion,
    Parameters.computeTravelTimeFor,
    Parameters.sectionType,
...
const getRouteRangeOperationSpec: coreClient.OperationSpec = {
  ...
  responses: {
    200: {
      bodyMapper: Mappers.GetRouteRangeResponse
    },
    default: {
      bodyMapper: Mappers.ErrorResponse
    }
...
const postRouteDirectionsBatchOperationSpec: coreClient.OperationSpec = {
  path: "/route/directions/batch/{format}",
  httpMethod: "POST",
  responses: {
    200: {
      bodyMapper: Mappers.RouteDirectionsBatchResponse
    },
    201: {
      bodyMapper: Mappers.RouteDirectionsBatchResponse
    },
...
const postRouteDirectionsBatchSyncOperationSpec: coreClient.OperationSpec = {
